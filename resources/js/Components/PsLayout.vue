      ...
      dark:bg-secondaryDark-black dark:text-textLight">
            ...
            <div @click="clickOutsideSidebar" :style="[!project.ps_license_code ? 'margin-top: 24px !important;' : '']"
                ...
                <div class="h-full px-4  transition-all duration-600 overflow-x-hidden overflow-y-auto scroll-smooth">
                    ...
                    <div class="pb-18">
                        <div >
                            <div v-if="isVersionUpdate" class="flex p-6 gap-4 bg-yellow-50 rounded shadow border-l-4 border-yellow-500 mb-4">
                                <div class="">
                                    <ps-icon class="text-primary-500" name="hond" w="32" h="32" viewBox="0 0 32 32"/>
                                </div>
                                <div class="w-full">
                                    <div class="flex justify-between w-full">
                                        <ps-label textColor="leading-6 text-base font-medium text-primary-500">{{ $t("core_be__version_update_noti_title") }}</ps-label>
                                        <ps-label textColor="leading-5 text-sm text-secondary-400 flex items-center gap-2 cursor-pointer" @click="isVersionUpdate = false">
                                            <ps-icon name="close-fill" w="16" h="16" viewBox="0 0 16 16"/>
                                            {{ $t("core_be__dismiss") }}
                                        </ps-label>
                                    </div>
                                    <ps-label textColor="text-sm text-secondary-500 mt-3">{{ $t("core_be__version_update_noti_desc") }}</ps-label>
                            ...
                            <div v-if="isProjectChanged" class="flex p-6 gap-4 bg-yellow-50 rounded shadow border-l-4 border-yellow-500 mb-4">
                                <div class="">
                                    <ps-icon class="text-primary-500" name="project" w="32" h="32" viewBox="0 0 32 32"/>
                                </div>
                                <div class="w-full">
                                    <div class="flex justify-between w-full">
                                        <ps-label textColor="leading-6 text-base font-medium text-primary-500">{{ $t("core_be__project_change_noti_title") }}</ps-label>
                                        <ps-label textColor="leading-5 text-sm text-secondary-400 flex items-center gap-2 cursor-pointer" @click="isProjectChanged = false">
                                            <ps-icon name="close-fill" w="16" h="16" viewBox="0 0 16 16"/>
                                            {{ $t("core_be__dismiss") }}
                                        </ps-label>
                                    </div>
                                    <ps-label textColor="text-sm text-secondary-500 mt-3">{{ $t("core_be__project_change_noti_desc") }}</ps-label>
                            ...
                            <div v-if="isConnected" class="flex p-6 gap-4 bg-yellow-50 rounded shadow border-l-4 border-yellow-500 mb-4">
                                <div class="">
                                    <ps-icon class="text-primary-500" name="connection" w="32" h="32" viewBox="0 0 32 32"/>
                                </div>
                                <div class="w-full">
                                    <div class="flex justify-between w-full">
                                        <ps-label textColor="leading-6 text-base font-medium text-primary-500">{{ $t("core_be__losing_connection_noti_title") }}</ps-label>
                                        <ps-label textColor="leading-5 text-sm text-secondary-400 flex items-center gap-2 cursor-pointer" @click="isConnected = false">
                                            <ps-icon name="close-fill" w="16" h="16" viewBox="0 0 16 16"/>
                                            {{ $t("core_be__dismiss") }}
                                        </ps-label>
                                    </div>
                                    <ps-label textColor="text-sm text-secondary-500 mt-3">{{ $t("core_be__losing_connection_noti_desc") }}</ps-label>
                                    <ps-button class="mt-6" rounded="rounded">
...
export default defineComponent({
    components: { TitleBar, SidebarMenu, Head, Link, PsLabel, PsIcon, PsButton, PsSuccessDialog, PsLicenseActivateModal },
    props: [
        "title",
        "project",
        'firebaseConfig',
