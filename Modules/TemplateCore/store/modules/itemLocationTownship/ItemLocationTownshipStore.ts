...
defineStore(`ItemLocationTownshipStore/${key}`,
 () => {
    ...
    async function resetLocationTownshipList(loginUserId: string, holder: LocationTownshipParameterHolder) {
        ...
        const responseData = await PsApiService.getItemSubLocationList<ItemLocationTownship>(new ItemLocationTownship(), limit.value, offset, loginUserId, holder.toMap());
    ...
    async function resetItemLocationTownshipList(locationId: string) {
