<template>
    <div class="w-full lg:rounded-lg sm:rounded-xl rounded-2xl sm:flex mt-4 sm:mt-0 gap-5">
        <div class="flex gap-x-5 flex-col md:flex-row gap-y-2">
                ...
                <ps-input-with-left-icon theme="dark_text-feSecondary-200" placeholderColor="" rounded="rounded" defaultBorder="bg-feSecondary-50 dark_bg-feSecondary-800 border border-feSecondary-200 hover_border-feSecondary-400 dark_border-feSecondary-200 hover:dark_border-feSecondary-50 focus:outline-none focus_border-none focus_ring-2 focus_ring-fePrimary-300 ring-fePrimary-300 placeholder-feSecondary-500 dark_placeholder-feSecondary-200" padding="h-10" v-on:keyup.enter="searchClicked"  v-model:value="searchKeyword" :placeholder="$t('dashboard__enter_keyword')" >
                    <template #icon>
                ...
                <ps-dropdown align="left" class='w-full md:w-52 h-full lg:rounded-2xl rounded' @onClick="loadSubLocation()">
                    <template #select>
                        <ps-label class="lg:rounded-2xl rounded-xshadow-sm h-full">
                            <button
                            type="button"
                            class="focus_ring-2 ring-fePrimary-200 inline-flex items-center content-start justify-between w-full h-10 lg:rounded rounded-x
                            px-4  bg-feSecondary-000 text-sm leading-5 font-medium text-feSecondary-500 dark_text-feSecondary-200
                            hover_text-feSecondary-400 focus:outline-none border border-feSecondary-200 focus_border-fePrimary-500
                            ...
                            aria-haspopup="true"
                            aria-expanded="true">
                            <!-- <ps-icon textColor="text-feSecondary-400 dark_text-feAchromatic-500" name="location" /> -->
                            <ps-label v-if="categoryName" textColor="font-normal text-feSecondary-500 dark_text-feSecondary-200" class=" text-start"> {{categoryName}} </ps-label>
                            <ps-label v-else textColor="font-normal text-feSecondary-500 dark_text-feSecondary-200"> {{ $t('choose_categories') }} </ps-label>
                            <ps-icon textColor="text-feSecondary-400 dark_text-feSecondary-200" name="downArrow"   />
                            </button>
                        </ps-label>
                    </template>
                    <template #filter >
                        <div>
                            <ps-input-with-left-icon rounded="rounded-t-lg" theme="ring-fePrimary-200 dark_ring-feAchromatic-500 focus:outline-none focus_ring focus_border-fePrimary-200 border-feSecondary-300 lg:rounded rounded-x bg-feSecondary-200 dark_bg-feAchromatic-800 text-feSecondary-500 dark_text-feAchromatic-50" height="h-10" @keyup.enter="filtersubLocationUpdate(categoryStore.filterKeyword)" v-model:value="categoryStore.filterKeyword" v-bind:placeholder= "$t('search_categories')" >
                                <template #icon>
                                    <ps-icon textColor="text-fePrimary-500 dark_text-feAchromatic-500" name="search" class='cursor-pointer' @click="filtersubLocationUpdate(categoryStore.filterKeyword)" />
                                </template>
                            </ps-input-with-left-icon>
                        </div>
                    </template>
                    <template #list >
                        ...
                        <div class="pt-2 z-30">
                            ...
                            <div v-else class="w-56 flex py-4 px-2 hover_bg-fePrimary-50 dark_hover_bg-feAchromatic-800 cursor-pointer items-center"  @click="categoryFilterClicked('',$t('item_list__all'))" >
                                <ps-label class="ms-2" :class="categoryId == '' ? 'font-bold text-fePrimary-700' : ''"  > All </ps-label>
                            </div>
                            <div v-for="selectData in categoryStore.itemList.data" :key="selectData.catId" class="w-56 flex py-4 px-2 hover_bg-fePrimary-50 dark_hover_bg-feAchromatic-800 cursor-pointer items-center"  @click="categoryFilterClicked(selectData.catId, selectData.catName)" >
                ...
                <ps-dropdown :id="locationId ?? 'locaitonid'" align="left" class='w-full md:w-52 h-full lg:rounded-2xl rounded' @onClick="loadLocationList" >
                    <template #select>
                        <ps-label class="lg:rounded-2xl rounded-x h-full">
                            ...
                            aria-expanded="true">
                            <!-- <ps-icon textColor="text-feSecondary-400 dark_text-feAchromatic-500" name="location" /> -->
                            <ps-label v-if="locationName" textColor="font-normal text-feSecondary-500 dark_text-feSecondary-200" class=" text-start"> {{locationName}} </ps-label>
                            <ps-label v-else textColor="font-normal text-feSecondary-500 dark_text-feSecondary-200"> {{ $t('choose_location') }} </ps-label>
                            <ps-icon textColor="text-feSecondary-400 dark_text-feSecondary-200" name="downArrow"   />
                            </button>
                        </ps-label>
                    </template>
                    <template #filter >
                        <div>
                            <ps-input-with-left-icon rounded="rounded-t-lg" theme="ring-fePrimary-200 dark_ring-feAchromatic-500 focus:outline-none focus_ring focus_border-fePrimary-200 border-feSecondary-300 lg:rounded rounded-x bg-feSecondary-200 dark_bg-feAchromatic-800 text-feSecondary-500 dark_text-feAchromatic-50" height="h-10" @keyup.enter="filterKeywordUpate(itemLocationStore.filterKeyword)"  v-model:value="itemLocationStore.filterKeyword" v-bind:placeholder= "$t('search_location')" >
                                <template #icon>
                ...
                <div class="flex justify-center">
                    <ps-button v-bind:title="$t('search_for_large_screem__search')" class="px-8 py-2 md:px-5 lg:px-8" rounded="rounded" theme="bg-fePrimary-500" @click="searchClicked()">
                        <ps-label class="text-sm font-medium" textColor="text-feAchromatic-50 dark_text-feSecondary-50"> {{ $t("search_for_large_screem__search") }} </ps-label>
                    </ps-button>
                </div>
...
export default {
    ...
    setup( props, context ) {
        ...
        function filtersubLocationUpdate(value) {
            ...
            categoryStore.resetCategoryList(loginUserId,subparamHolder);
        }
