        ...
        <template #content>
            <div class="mt-32 mb-10">
                ...
                <div class="mt-8">
                        ...
                        <div class="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-6">
                            <div class="w-full" v-for="user in userStore.userList?.data" :key="user.userId">
                                <!-- <ps-route-link :to="{ name: 'fe_other_profile', params: { userId: user.userId }}"> -->
                                    <user-list-horizontal storeName="userSearch" :user="user" />
                                <!-- </ps-route-link> -->
                            </div>
                        </div>
                        <div class="flex flex-col items-center">
                            <ps-button v-if="userStore.loading.value == false && initial == false" class="font-medium mx-auto mt-6" @click="loadMore" :class="userStore.isNoMoreRecord.value ? 'hidden' : ''"> {{ $t("follower_list__load_more") }} </ps-button>
                    ...
                    <ps-no-result v-if="userStore.loading.value == false && userStore.userList?.data == null && initial == false" @onClick="loadMore" />
                </div>
            </div>
        </template>
    </ps-content-container>
...
export default {
    ...
    setup(props) {
        ...
        // userStore.loadUserSearchList( loginUserId,userStore.userparamHolder);
