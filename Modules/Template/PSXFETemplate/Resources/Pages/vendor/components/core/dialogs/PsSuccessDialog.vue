<template>
    <ps-modal ref="psmodal" maxWidth="450px" line="hidden" :isClickOut='false' theme=" px-6 py-7 rounded-lg shadow-xl" class=' z-20'>
        ...
        <template #title>
            <ps-icon v-if="showIcon" @click="close()" name="close" class="text-sm text-feSecondary-400 ms-auto my-auto focus:shadow-none hover_text-purple-500 flex justify-end"  />
        ...
        <template #body>
            <div class="w-full text-center mt-4">
                <ps-label class="font-light text-sm text-feSecondary-700 lg:text-base"> {{message}} </ps-label>
            </div>
        </template>
        <template #footer>
            <div class=" flex justify-center mt-6">
                <div class="flex-grow-0">
                    <ps-button rounded="rounded" @click="actionClicked()" textSize="text-sm" class="" colors="bg-fePrimary-500 text-feAchromatic-50"  hover="hover:outline-none hover_ring hover_ring-feSuccess-100" focus="focus:outline-none focus_ring focus_ring-feSuccess-300" > {{okButton}} </ps-button>
                </div>
