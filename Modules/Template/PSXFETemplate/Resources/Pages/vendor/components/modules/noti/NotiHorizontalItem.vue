<template>
    <div :class="noti.isRead == '0' ? noti.type == PsConst.PUSH_NOTI ? 'bg-feSecondary-100' : 'bg-fePrimary-50 dark_bg-fePrimary-50' : 'bg-feSecondary-50'" class="w-full cursor-pointer h-[62px] sm:h-[100px]  dark_bg-feAchromatic-800 rounded" v-on:click="onClick != null ? onClick(noti) : null">
        <div class="flex flex-row justify-between h-[62px] sm:h-[100px]">
            <div class="flex-grow flex flex-row" @click="clickNoti(noti)">
                <div class="rounded-md bg-feAchromatic-50 dark_bg-feAchromatic-800">
                ...
                <div class="flex flex-col ms-4 my-auto">
                    <ps-label class="text-xxs sm:text-sm lg:text-base" :textColor="noti.isRead == '0' ? noti.type == PsConst.PUSH_NOTI ? 'text-feAchromatic-500 dark_text-feAchromatic-200' : 'text-feAchromatic-500' : 'text-feAchromatic-500 dark_text-feAchromatic-200'" v-html=" noti.message.length > 75 ? noti.message.slice(0,75)+' ....' : noti.message "> </ps-label>
                    <!-- <ps-label class="hidden sm:block text-xxs sm:text-xxs lg:text-xs mt-1" :textColor="noti.isRead == '0' ? noti.type == PsConst.PUSH_NOTI ? 'text-feAchromatic-500 dark_text-feAchromatic-200' : 'text-feAchromatic-500' : 'text-feAchromatic-500 dark_text-feAchromatic-200'"  v-html=" noti.description.length > 100 ? noti.description.slice(0,95)+' ....' : noti.description "> </ps-label> -->
                </div>
            ...
            <div class="flex flex-col w-[62px] sm:w-[120px]">
               <div class="flex justify-end w-full h-1/2">
                    <ps-dropdown horizontalAlign="right" h="h-auto " class='' @click="showMenu = !showMenu" >
                        <template #select>
                            <ps-icon  name="vertical-dot" w="24" h="24" :textColor="noti.isRead == '0' ? noti.type == PsConst.PUSH_NOTI ? 'text-feAchromatic-500 dark_text-feAchromatic-200' : 'text-feAchromatic-500' : 'text-feAchromatic-500 dark_text-feAchromatic-200'" class=" mt-2 text-base me-1 cursor-pointer" />
                        </template>

                        <template #list >

                            <div class="z-30">
                                <div @click="markAsRead(noti.id, noti.type)" v-if="noti.isRead == '0' && noti.type != PsConst.PUSH_NOTI" class='w-56 flex justify-center items-center py-4 px-2 hover_bg-fePrimary-50 dark_hover_bg-feAchromatic-800 cursor-pointer'>
                                    <ps-icon name="checkNoCircle" class="mt-2"></ps-icon>
                                    <ps-label class="ms-2" > {{ $t("noti_horizontal_item__mark_as_read") }} </ps-label>
                                </div>
                                <div @click="markAsUnRead(noti.id, noti.type)"  v-if="noti.isRead == '1'" class='w-56 flex justify-center py-4 px-2 hover_bg-fePrimary-50 dark_hover_bg-feAchromatic-800 cursor-pointer items-center'>
                                    <ps-icon name="checkNoCircle" class="mt-2"></ps-icon>
                                    <ps-label class="ms-2" > {{ $t("noti_horizontal_item__mark_as_unread") }} </ps-label>
                                </div>
                                <ps-line class="mx-4"/>
                                <div  @click="deleteConfirm(noti)" class="w-56 flex justify-center py-4 px-2 hover_bg-fePrimary-50 dark_hover_bg-feAchromatic-800 cursor-pointer items-center"  >
               ...
               <div class="h-1/2 w-full flex justify-end items-end p-1 sm:p-2">
                    <ps-label class="text-3xs sm:text-xs truncate" :textColor="noti.isRead == '0' ? noti.type == PsConst.PUSH_NOTI ? 'text-feAchromatic-500 dark_text-feAchromatic-200' : 'text-feAchromatic-500' : 'text-feAchromatic-500 dark_text-feAchromatic-200'">{{ moment(noti.addedDate).format($page.props.dateFormat) }}</ps-label>
               </div>
            </div>
        </div>
    ...
    setup() {
        ...
        async function clickNoti(noti){
            if(noti.type == PsConst.PUSH_NOTI){
                Inertia.get(route('fe_notification_detail'),{'notiId' : noti.id });
