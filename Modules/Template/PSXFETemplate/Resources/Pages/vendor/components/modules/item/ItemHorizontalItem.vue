<template>
    <div class="cursor-pointer w-full h-full pb-3" v-on:click="onClick != null ? onClick(item) : null">
        ...
        <ps-card class="flex w-full bg-feAchromatic-50 flex-col lg:rounded-lg shadow-sm "
            ...
            <!-- <div v-if="psValueStore.showProfile == 'show'" class="flex items-center justify-between leading-none lg:px-2 py-1 px-1  bg-fePrimary-50 dark_bg-feAchromatic-800 rounded-t-xl lg:rounded-t-2xl">
                <div class="flex flex-row items-center no-underline text-feAchromatic-900">
                    ...
                    <p class="ms-2 mt-1 text-sm flex-grow">
                        ...
                        <ps-label class=" font-medium text-xxs lg:text-xs " textColor="text-fePrimary-500 dark_text-feAccent-500" v-if="item != null && item.paidStatus == PsConst.PAID_AD_PROGRESS"> {{ $t('item_horizontal_item__sponsored') }} </ps-label>
                        <ps-label class=" font-medium text-xxs lg:text-xs " textColor="text-feSecondary-400 dark_text-feAchromatic-500"  v-else> {{ item ? item.addedDateStr : '' }}  </ps-label>
                    </p>
                </div>
            </div>  -->
            <div class="lg:px-0 lg:py-0 px-1 "
                ...
                <div class="h-44 relative z-0">
                    ...
                    <div class="absolute z-10">
                        <ps-route-link :to="{ name: 'fe_item_detail', query: { item_id: item.id } }">
                            <div class=" overflow-hidden h-44 w-full" >
                                <img alt="Placeholder" class="transform transition duration-500 hover:scale-110 rounded-t-lg object-cover  w-screen h-44 " width="640px"
                                height="360px"
                    ...
                    <div v-if="!notShowTitle" class="absolute z-20 bg-feAchromatic-50 dark_bg-feSecondary-800 rounded w-10 h-10 flex justify-center items-center right-2 top-2" @click="favouriteClicked(item.id)">
                        <ps-icon v-if="item.isFavourited == '1'" textColor="text-fePrimary-500 dark_text-feAchromatic-50" class="" name="heart" h="24" w="24" />
                    ...
                    <div v-if="item.status == '1'" >
                        <div v-if="item.isDiscount == '1'"
                            ...
                            <div v-if="item.isPaid == '1' && item.paidStatus == PsConst.paidItemProgressStatus" class="absolute bg-feSuccess-500 rounded px-2 py-1 left-0 top-8">
                                <ps-label class="text-xs lg:text-sm font-medium text-center"
                                    textColor="text-feAchromatic-50 "> {{ $t('item_horizontal_item__featured') 
                                    }}</ps-label>
                            </div>
                        </div>
                        <div v-else-if="item.isPaid == '1' && item.paidStatus == PsConst.paidItemProgressStatus"
                    ...
                    <!-- <div class="relative opacity-80 bg-fePrimary-500 dark_bg-feAccent-500 rounded-b-lg">
                        <ps-label v-if="item && item.isSoldOut == '1'" class="font-medium text-sm lg:text-base lg:py-1.5 py-2 rounded-b-lg text-center" textColor="text-feAchromatic-50 dark_text-feAchromatic-900"> {{ $t('item_horizontal_item__sold_out') }}</ps-label>
                    </div> -->
                ...
                <ps-route-link :to="{ name: 'fe_item_detail', query: { item_id: item.id } }">
                    <div class="ps-1 pt-1" v-if="!notShowTitle">
                        <ps-label class="truncate-2-lines mb-0.5 font-normal text-sm" textColor="text-feSecondary-600 dark_text-feSecondary-200">{{ item ? item.title :
                            '' }}</ps-label>
                        <!-- <ps-label class="ms-2 flex-grow font-light text-xxs lg:text-xs" textColor="text-fePrimary-500 dark_text-feAccent-500"> {{item ? item.conditionOfItem.name : ''}} </ps-label>   -->
                    </div>
                    <!-- <div v-if="item?.discountRate!='0' && appInfoStore.appInfo.data.psItemUploadConfig.discountRate== PsConst.ONE" class="flex flex-col ">

                    <ps-label class=" pt-0.5 ps-1 font-medium text-xs lg:text-base" textColor="text-fePrimary-500 dark_text-feAccent-500"> {{ item ? item.itemCurrency.currencySymbol : '' }} {{ formatPrice(item ? parseFloat(item.price)-(parseFloat(item.price)*(parseFloat(item.discountRate)/100)) : '') }} </ps-label>
                    ...
                    <div :class="showProfile ? 'pb-0' : 'pb-2'" class="ps-1 flex justify-between" v-if="!notShowTitle">
                        <div class="flex items-center  rtl:space-x-reverse space-x-2.5">
                            ...
                            <ps-label class="truncate font-normal text-sm"
                                textColor="text-feSecondary-400 dark_text-feSecondary-200"> {{ item ?
            ...
            <ps-route-link :to="{ name: 'fe_other_profile', params: { userId: item?.user?.userId }}" >
                <div v-if="showProfile" class="pt-1 pb-2 flex items-center justify-between leading-none px-2  rounded-b-xl lg:rounded-b-2xl">
                    <div class="flex flex-row items-center no-underline text-feAchromatic-900">
                        ...
                        <p class="ms-2 mt-1 text-sm flex-grow">
                            <span class="flex">
                                <ps-label class="truncate text-xs" textColor="text-feSecondary-800 dark_text-feSecondary-300" > {{ item.user.userName.length > 12 ? item.user.userName.slice(0,12)+' ...' : item.user.userName }} </ps-label>
                            ...
                            <ps-label class=" font-light text-xs " textColor="text-feSecondary-500 dark_text-feSecondary-500"> {{ item ? moment(item.addedDate).format($page.props.dateFormat) : '' }}  </ps-label>
                        </p>
                    </div>
                </div>
            </ps-route-link>
...
<style scoped>
.truncate-2-lines {
  height: 2.8em; /* double the size of line-height */
  line-height: 1.4em;
  overflow:hidden;
