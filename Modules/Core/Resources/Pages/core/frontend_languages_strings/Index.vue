<template>
    <Head :title="$t('fe_language_module')" />
    <ps-layout>
        <!-- breadcrumb start -->
        ...
        <ps-table2 :row="row" :search="search" :object="fe_language_strings" :colFilterOptions="colFilterOptions"
            ...
            <template #button>
                   <ps-button v-if="can.createFeLanguageString" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" /> {{$t('create_fe_language_string')}}</ps-button>
            </template>
             <template #responsive_button>
                   <ps-button v-if="can.createFeLanguageString" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" />  {{ $t('create_fe_language_string') }}</ps-button>
            </template>
            <template #searchLeft>
                <ps-button v-if="can.createFeLanguageString" rounded="rounded" @click="csvUploadClicked" class="ms-3 flex flex-wrap items-center ">
                ...
                <a :href="route('fe_language_string.export.json', language.id)">
                ...
                <a :href="route('fe_language_string.export.csv', language.id)">
                    <ps-button v-if="can.createLanguageString" rounded="rounded" colors="text-primary-500" border="border border-primary-500" class="ms-4 flex flex-wrap items-center ">
                        <ps-icon name="fileUpload" class="me-2 font-semibold" />
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function confirmDeleteClicked(id) {
            ps_danger_dialog.value.openModal(
               trans('core__be_delete_felanguagestring'),
                trans('core__be_delete_felanguagestring_info'),
                trans('core__be_btn_confirm'),
                trans('btn_cancel'),
                () => {
                    this.$inertia.delete(route("fe_language_string.destroy", [props.language, id]),{
                        onSuccess: () => {
                            visible.value = true;
                            setTimeout(() => {
                                visible.value = false;
        ...
        function csvUploadClicked(){
            ps_csv_modal.value.openModal(
                (selectedFile) => {
                    ...
                    Inertia.post(route('fe_language_string.import.csv', props.language), form, {
                        onSuccess: () => {
        ...
        function handleSearchingSorting(page = null, row = null) {
            Inertia.get(route('fe_language_string.index',props.language.id),
                {
                    sort_field: sort_field.value,
                    sort_order: sort_order.value,
                    page: page ?? props.fe_language_strings.meta.current_page,
                    row: row ?? props.fe_language_strings.meta.per_page,
                    search: search.value,
                },
                {
                    preserveScroll: true,
    ...
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                {
                    label: trans('fe_language_module'),
                    url: route('fe_language.index')
                },
                {
                    label: trans('fe_language_string_module'),
    ...
    methods: {
        handleCreate() {
            this.$inertia.get(route('fe_language_string.create',this.language.id));
        },
        handleEdit(id){
            this.$inertia.get(route('fe_language_string.edit',[this.language,id]));
        },
        FilterOptionshandle(value) {

            Inertia.post(route('fe_language_string.screenDisplayUiSetting.store'),
                {
                    value,
                    sort_field: this.sort_field,
                    sort_order: this.sort_order,
                    row: this.fe_language_strings.per_page,
                    search: this.search.value,
                    current_page: this.fe_language_strings.current_page
                },
                {
                    preserveScroll: true,
                    preserveState: true,
