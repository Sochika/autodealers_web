                        ...
                        <div class="grid xxl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1">
                            <div class="grid gap-6">
                                ...
                                <div>
                                    ...
                                    <Dropzone :dir="$page.props.dir"  @clicked="pushImage" @removeImage="removeImage"  @caption="caption" :images="form.item_image" :edit_mode="1" :item_id="item.id" :max_image_upload="this.systemConfig.max_img_upload_of_item"  />
                         ...
                         <div>
                                ...
                                <div>
                                    ...
                                    <div v-if="item.icon[0]" class="flex items-end pt-4">
                                        ...
                                        <ps-button type="button" @click="replaceIconClicked(item.icon[0].id,'item')" rounded="rounded-full" shadow="drop-shadow-2xl" class="-ms-10 mb-2" colors="bg-white text-primary-500 dark:bg-secondaryDark-black" border="border border-1 dark:border-secondary-700 border-secondary-300" padding="p-1.5" hover="" focus="">
                                            <ps-icon name="pencil-btn"  w="21" h="21" />
                                        </ps-button>
                                        <ps-image-icon-modal ref="ps_image_icon_modal" />
                                        <ps-action-modal ref="ps_action_modal" />
                            ...
                            <div class="w-full ms-4">
                                ...
                                <div class="mb-6" v-for="(custom_field_header) in customizeHeaders.filter((custom) => custom.core_keys_id == 'ps-itm00009')" :key="custom_field_header.id">
                                    ...
                                    <div class="md:me-6 sm:me-0" v-if="custom_field_header.ui_type_id === 'uit00008' && custom_field_header.enable === 1 && custom_field_header.is_delete === 0">
                                        <ps-label class="text-base">{{ $t(custom_field_header.name) }}<span class="text-red-800 font-medium ms-1" v-show="custom_field_header.mandatory === 1">*</span></ps-label>
                                        <div class="flex flex-row">
                                            <CheckBox :oldData="form.product_relation[custom_field_header.id]" @toParent="handleMultiSelect" :customizeDetails="customizeDetails" :customizeHeader="custom_field_header" />
                                        </div>
                                        <ps-label-caption textColor="text-red-500 " class="mt-2 block">{{ product_relation_errors && product_relation_errors[custom_field_header.core_keys_id] }}</ps-label-caption>
                                    </div>
                                    ...
                                    <div class="md:me-6 sm:me-0" v-if="custom_field_header.ui_type_id === 'uit00009' && custom_field_header.enable === 1 && custom_field_header.is_delete === 0">
                                        ...
                                        <div v-if="item.image" class="flex items-end pt-4">
                                            ...
                                            v-lazy=" { src: $page.props.uploadUrl + '/' + item.image.img_path, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_photo.png' }"
                                             class="w-96 h-48" :alt="$t(core__be_item_photo)" />
                                            <ps-button type="button" @click="replaceImageClicked(item.image.id,'item')" rounded="rounded-full" shadow="drop-shadow-2xl" class="-ms-10 mb-2" colors="bg-white text-primary-500 dark:bg-secondaryDark-black" border="border border-1 dark:border-secondary-700 border-secondary-300" padding="p-1.5" hover="" focus="">
                                                <ps-icon name="pencil-btn"  w="21" h="21" />
                                            </ps-button>
                                            <ps-image-icon-modal ref="ps_image_icon_modal" />
                                            <ps-action-modal ref="ps_action_modal" />
                                ...
                                <div class="mb-6" v-else>
                                    <open-street-map-pin-picker :dir="$page.props.dir" :onChange="setCoordinates" :lat="parseFloat(form.lat)" :lng="parseFloat(form.lng)" class="h-[500px]"></open-street-map-pin-picker>
                                </div>
                                <!-- maps end -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        // alert(usePage().props.value.uploadUrl);
         ...
         function handleFilePondInit() {
            ...
            if(form.item_image)
            {
                ...
                for(let i=0;i<arr.length;i++){
                    ...
                    myFiles.value.push({
                        source:usePage().props.value.uploadUrl +'/'+arr[i],
                        options:{
                            type:'local',
                            metadata: {
                                poster: usePage().props.value.uploadUrl +'/'+arr[i],
    ...
    methods: {
        ...
        //     {
        //         this.myFiles=[{
        //             source: usePage().props.value.uploadUrl +'/'+this.form.item_image,
        //             // options: {"type":"local"}
        //             // options:{
        //             //     type:'local',
        //             //     metadata:{
        //             //         poster: usePage().props.value.uploadUrl + '/'+this.form.item_image
        //             //     }
        //             // }
        //         }]
        //     }
